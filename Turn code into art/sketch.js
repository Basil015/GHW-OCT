...
for (var i = 0; i < YinYangArray.length; i += 1) {
    
    var ArrayColors = YinYangArray[i];
    
    for (var c = 0; c < YinYangArray[i].length; c += 1){
       
        var rectSize = 9;
        noStroke();
      
        
        var Pixel = ArrayColors[c];
        switch(Pixel) {
        ...
        case "" : 
            fill(0, 0, 0);
                text("YOU ROCK",106,63);
            break;
            case "_" :fill(255, 242, 0); //Purple
            break;
            case "8" : fill(255, 255, 255); //yellow
            break;
            
        
        }
            rect((c * rectSize) + x, (i * rectSize) + y, rectSize+1, rectSize+1);
    }
}
